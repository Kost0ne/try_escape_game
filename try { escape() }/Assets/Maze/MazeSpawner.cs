...
public class MazeSpawner : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        GameObject zakuska;
        if (finPosition.x == 0)
            zakuska = Instantiate(zakuskaPrefab, new Vector3(finPosition.x, finPosition.y + CellSize.y / 2, 0) + positionDelta,
                Quaternion.Euler(0, 0, 90));
        else if (finPosition.y == 0)
            zakuska = Instantiate(zakuskaPrefab, new Vector3(finPosition.x + CellSize.x / 2, finPosition.y, 0) + positionDelta,
                Quaternion.identity);
        
        else if (finPosition.x == generator.Width - 2)
            zakuska = Instantiate(zakuskaPrefab, new Vector3(finPosition.x + CellSize.x, finPosition.y + CellSize.y / 2, 0) + positionDelta,
                Quaternion.Euler(0, 0, 90));
        else //(finPosition.y == generator.Height - 2)
            zakuska = Instantiate(zakuskaPrefab, new Vector3(finPosition.x + CellSize.x / 2, finPosition.y + CellSize.y, 0) + positionDelta,
                Quaternion.identity);
        //zakuska.tag = "zakuska";
    }
}